import type { IssuerAdapter } from "../adapter.js";
import { SECONDS_PER_DAY, delay, randomRef } from "./shared.js";

/**
 * MOCK adapter for Franklin (Franklin Templeton's BENJI fund, PRD §5 Phase 1 asset #1 runner-up —
 * "Franklin BENJI uses native Stellar controls"). Real integration is Phase 0 work; every
 * response here is a plausible fake, not a live call to Franklin's transfer agent.
 */
export class MockFranklinAdapter implements IssuerAdapter {
  /** ASSUMPTION: BENJI's native Stellar controls make same-day/T+1 redemption plausible. */
  private readonly redemptionLagDays = 1;

  async requestAuthorization(assetCode: string, holderAddress: string): Promise<{ approved: boolean; ref: string }> {
    await delay(120);
    void assetCode;
    void holderAddress;
    return { approved: true, ref: randomRef("franklin-auth") };
  }

  async requestSep8Approval(
    assetCode: string,
    txXdr: string,
  ): Promise<{ approved: boolean; approvedTxXdr?: string; reason?: string }> {
    await delay(250);
    void assetCode;
    return { approved: true, approvedTxXdr: txXdr };
  }

  async requestLienFreeze(
    assetCode: string,
    holderAddress: string,
    units: bigint,
  ): Promise<{ lienRef: string }> {
    await delay(180);
    void assetCode;
    void holderAddress;
    void units;
    return { lienRef: randomRef("franklin-lien") };
  }

  async requestLienRelease(assetCode: string, lienRef: string): Promise<void> {
    await delay(120);
    void assetCode;
    void lienRef;
  }

  async requestRedemption(
    assetCode: string,
    units: bigint,
    destination: string,
  ): Promise<{ redemptionRef: string; expectedSettlementAt: number }> {
    await delay(200);
    void assetCode;
    void units;
    void destination;
    const expectedSettlementAt = Math.floor(Date.now() / 1000) + this.redemptionLagDays * SECONDS_PER_DAY;
    return { redemptionRef: randomRef("franklin-redeem"), expectedSettlementAt };
  }
}
